...
def wordlify(word: str, wordle_output: str) -> str:
    cases: list[str] = ["🟥","🟨","🟩"]
    length = len(word) + 4
    wordle_output = ''.join(map((lambda x: cases[int(x)]), list(wordle_output)))
    return rf"""
        /{"="*length}\/{"="*(length + 5)}\
...
def word_filter(numerical_representation: list[int], ignore: list[int], letters: list[int], must: list[tuple[int, int]]):
...
def main() -> None:
    ...
    if len(phrase) != len(output):
        exit()
    log: list[tuple[str, str]] = [(output, phrase)]
    start_time = time_ns()
